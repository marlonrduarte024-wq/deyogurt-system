...
export default function Items(){
    ...
    async function guardarItem(){
        ...
        if(id){
            await supabase
                .from("items")
                .update({ nombre, precio_venta: Number(precioVenta) })
                .eq("id",id)
            ...
            const {data:item} = await supabase
                .from("items")
                .insert([{ nombre, precio_venta: Number(precioVenta) }])
                .select()
                .single()
        ...
        for(const r of receta){
            ...
            await supabase
                .from("recetas")
                .insert([{ item_id:itemId, insumo_id:r.insumo_id, cantidad:Number(r.cantidad) }])
        }
